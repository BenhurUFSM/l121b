...
1. Faça uma função que compara datas; ela recebe duas datas (como a estrutura do exemplo acima) e retorna um bool, que é verdadeiro se a primeira data for anterior à segunda.
   ...
   *Solução vista em aula:*
   ```c
   bool compara_datas(data_t d1, data_t d2)
   {
     // retorna verdadeiro se d1 < d2
     if (d1.ano < d2.ano) {
       return true;
     }
     if (d1.ano > d2.ano) {
   ...
   ```
3. Faça uma função que retorna a nota do aluno mais novo. Essa função deve usar a função de comparação de datas.
   
   *Uma das soluções vistas em aula (alterada para usar uma variável auxiliar):*
   ```c
   double nota_do_mais_novo(int n, pessoa alunos[n])
   {
     pessoa *novo = alunos; // ponteiro para o primeiro aluno, chuta que seja o mais novo (poderia ser &alunos[0])
     pessoa *teste = alunos+1; // ponteiro para o segundo aluno, será usado para testar cada aluno (poderia ser &alunos[1])
     for (int i = 0; i < n-1; i++) {   // tem que testar n-1 alunos
       if (compara_datas(novo->nascimento, teste->nascimento)) {
         // se o nascimento do mais novo encontrado até agora for anterior à do teste, teste é mais novo ainda
         novo = teste;
       }
       teste++; // avança para o próximo aluno (poderia ser teste = &alunos[i+2])
     }
     return novo->nota;
   }
   ```
5. Refaça os dois exercícios acima, mas a função de comparação de datas deve ser alterada para receber dois ponteiros para data, e não duas datas.
6. Refaça o programa todo, modificando o controle do número de alunos. Em vez de se manter (e passar para as funções) uma variável contendo quantos alunos existem, um aluno com o nome vazio (`nome[0] == '\0'`) é usado para marcar o final dos alunos no vetor.
