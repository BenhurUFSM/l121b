...
O algoritmo de busca da saída é:
   ...
   2. enquanto houver posição guardada e não tiver encontrado a saída
      ...
      5. guarda as posições vizinhas que estiverem dentro do labirinto e que estejam livres
...
Uma posição no labirinto e representada por dois inteiros, a linha e a coluna dessa posição.
...
Além do vetor, tem um inteiro que mantém o índice do vetor onde será colocada a próxima posição (tem o valor 0 se o vetor não contiver nenhuma posição).

No algoritmo acima, "guardar uma posição" significa colocar a linha e a coluna nos próximos índices desse vetor, e "pegar uma posição guardada" significa retirar do vetor os valores nos dois últimos índices ocupados do vetor.
Faça uma função para cada uma dessas operações (essas funções devem receber o vetor e o índice do final, e necessitam alterar mais de uma variável, então devem usar ponteiros).
