...
Tendo incluído `stdio`, nosso programa pode usar um comando para chamar a função `printf`, que é a principal função de saída de dados da linguagem.
Esse comando permite imprimir caracteres no terminal.
Os caracteres que se deseja imprimir passados como argumento para a função, dentro dos parênteses, colocados entre aspas duplas (**"**).
...
Considere o programa C abaixo, que imprime o resultado de um cálculo complicado:
