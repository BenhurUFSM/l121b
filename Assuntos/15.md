...
2. Complemente o programa anterior para, além de imprimir a média móvel após a leitura de cada número, imprime ainda a "tendência" desses números. Essa tendência pode ser "em alta", "em baixa" ou "estável". A tendência é obtida comparando-se o valor da última média móvel com a média móvel calculada 14 valores antes, se a última é mais de 15% superior à antiga, a tendência é d alta; se for mais de 15% inferior à antiga a tendência é de baixa; caso contrário é estável. Caso uma das médias móveis necessárias não possa ser calculada, a tendência também não pode, e deve ser informada como "---".
